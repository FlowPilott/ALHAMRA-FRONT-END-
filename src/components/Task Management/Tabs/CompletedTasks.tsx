...
const CompletedTasks = ({ displayedRows }) => {
      ...
      <table className="w-full min-w-max border-separate border-spacing-y-2">
        ...
        <tbody>
          {displayedRows?.map((task, index) => (
            <tr
              key={index}
              className={`text-sm bg-white font-normal`}
            >
              ...
              <td className="px-2.5 py-4   text-[#1C1C1C]">
                {' '}
                {capitalizeFirstLetter(
                  task.template === 'Modification Request'
              ...
              <td
                className={`px-1.5 py-2   md:px-2.5 md:py-4 ${
                  task.status === 'Rejected' ? 'text-red' : 'text-[#1FCD67]'
                }`}
              >
                {task.status}
              </td>
