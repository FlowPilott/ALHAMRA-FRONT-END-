...
import UpdateBPNumberModal from '../UpdateBPNumberModal';

const ActiveTasks = ({ displayedRows, setMessage ,fetchTasks}) => {
  const [modalOpen, setModalOpen] = useState(false);
  const [selectedContractorId, setSelectedContractorId] = useState(null);
  const handleUpdateBPNumber = (contractorId:number) => {
    setSelectedContractorId(contractorId as unknown as null);
    setModalOpen(true);
  };
  ...
  return (
    <div className="scrollbar overflow-x-auto">
        ...
        <tbody>
          {displayedRows.length > 0
            ? displayedRows?.map((task, index) => (
                <tr key={index} className="text-sm bg-white font-normal">
                  <td className="px-1.5 py-1.5  text-[#1C1C1C] md:px-2.5 ">
                    {formatDate(task?.renewalDate)}
                  </td>
                  <td className="px-1.5 py-1.5text-[#1C1C1C] md:px-2.5 ">
                    {task.companyName}
                  </td>
                  <td className="cursor-pointer px-1.5 py-1.5 text-[#B2282F] md:px-2.5 ">
                    {task.address}
                  </td>
                  ...
                  <td className="px-1.5 py-1.5 text-[#1C1C1C] md:px-2.5 ">
                    <button
                      className="rounded bg-[#B2282F] px-4 py-2.5 text-white"
                      onClick={() => handleUpdateBPNumber(task)} // Pass the contractor's ID
                    >
                      Update BP Number
                    </button>
                  </td>
      ...
      {modalOpen && <UpdateBPNumberModal 
        setModalOpen={setModalOpen} 
        selectedContractorId={selectedContractorId || 0} 
